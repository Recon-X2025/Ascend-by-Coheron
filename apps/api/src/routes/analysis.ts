...
app.get("/jobs/:id", async (c) => {
  ...
  if (!parsed.success) return c.json({ success: false, data: undefined, error: "Invalid id" }, 400);
  const { id } = parsed.data;
  const job = await prisma.aIJob.findUnique({ where: { id } });
  if (!job) return c.json({ success: false, data: undefined, error: "Job not found" }, 404);
  return c.json({
    success: true,
    data: {
      id: job.id,
...
app.post("/jobs/parse-jd", async (c) => {
  ...
  if (!parsed.success) {
    ...
    return c.json({ success: false, data: undefined, error: msg }, 400);
  }
...
app.post("/jobs/analyze-resume", async (c) => {
  ...
  const user = c.get("user");
  if (!user) return c.json({ success: false, data: undefined, error: "Unauthorized" }, 401);
  const jobId = await enqueue(
    "resume_analysis",
